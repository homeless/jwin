var file_manager_lite = function() {
	...
	var NewWindow = function() {
		...
		var hWnd = CreateWindow("OurWindowClass", "File Manager Lite", WS_VISIBLE | WS_OVERLAPPEDWINDOW, GlobalNewWindowX, GlobalNewWindowY, 500, 340, 0, hMenu, 0, 0);
		SetMenu(hWnd, hMenu);
		return hWnd;	
	};
	...
	var OnMenuCommand = function(hWnd, id) {
		switch (id) {
			...
			case CMD_CUT:
				...
				var hwndEdit = GetDlgItem(hWnd, ID_LISTVIEW);
	...
	var OurWindowProc = function(hWnd, Msg, wParam, lParam) {
		switch (Msg) {
			...
			case WM_CREATE:
				...
				CreateWindowEx(WS_EX_CLIENTEDGE, "_jwin_fileListView", "", dwStyle, 20, 10, 170, 120, hWnd, ID_LISTVIEW, 0, 0);
				break;
			case WM_SIZE:
